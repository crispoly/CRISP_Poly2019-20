            ...
            if Datagram(2) == 5 % Commande SAP
                switch Datagram(3) 
                    case 177 % Start Current
                        Datagram = create_dg(motorID+1, 5, 177, 0, MatrixRead(motorID, 1));
                    case 4  % Max Velocity
                        Datagram = create_dg(motorID+1, 5, 4, 0, MatrixRead(motorID, 2));
                    case 172 % CoupleP
                        Datagram = create_dg(motorID+1, 5, 172, 0, MatrixRead(motorID, 3));
                    case 173 % CoupleI
                        Datagram = create_dg(motorID+1, 5, 173, 0, MatrixRead(motorID, 4));
                    case 234 % VelocityP
                        Datagram = create_dg(motorID+1, 5, 234, 0, MatrixRead(motorID, 5));
                    case 235 % VelocityI
                        Datagram = create_dg(motorID+1, 5, 235, 0, MatrixRead(motorID, 6));
                    case 230 % PositionP
                        Datagram = create_dg(motorID+1, 5, 230, 0, MatrixRead(motorID, 7));
                end
            end
