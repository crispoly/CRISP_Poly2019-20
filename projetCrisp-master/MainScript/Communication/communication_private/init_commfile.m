...
for i = 1:6 % initialisation des 6 contrôleurs
    ddl = num2str(i);
    if enabled_ddl(i)
        write_dg(map, strcat("pos_",ddl), get_sensor_dg(strcat("pos_",ddl)), 1);
        write_dg(map, strcat("vit_",ddl), get_sensor_dg(strcat("vit_",ddl)), 1);
        write_dg(map, strcat("acc_",ddl), get_sensor_dg(strcat("acc_",ddl)), 1);
